...
int LoRaClass::begin(long frequency) {
  ...
  // Kongduino: fill up the random buffer
  fillRandom(randomBuf, 256);
  // put in sleep mode
  sleep();
  // set frequency
...
uint8_t LoRaClass::getLoRandomByte() {
  uint8_t x = 0, b;
...
void LoRaClass::fillRandomMultiple(unsigned char *x, size_t len, uint8_t multiple) {
...
void LoRaClass::fillRandom(unsigned char *x, size_t len) {
  setupLoRandom();
  size_t i;
  for (i = 0; i < len; i++) {
    x[i] = getLoRandomByte();
...
void LoRaClass::fillRandom(unsigned char *x, size_t len, uint8_t except) {
...
void LoRaClass::fillRandom(unsigned char *x, size_t len, uint8_t minValue, uint8_t maxValue) {
